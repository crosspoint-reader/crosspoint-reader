...
description: "Device and PlatformIO-env facts for CrossPoint. Use when the task mentions a board, env, FREEINK_DEVICE_*, PSRAM / BOARD_HAS_PSRAM, panel size, framebuffer, controller, touch, frontlight, uiScale, bezel insets, or 'does this work on Sticky / X3 / X4'; when adding a board or compile-time cap; or when AGENTS.md defers a hardware number here."
---
...
3. **Load resources.** For each `FREEINK_DEVICE_*` on that compile set, read
   `resources/<device>.md`. Do not glob `resources/` and treat every file as
   live — only those devices. Today `[env:default]` (and the C3 release aliases)
   loads **both** `x3.md` and `x4.md`. They share one C3 binary until that env
   is split.
...
| Shared-binary aliases? | same flags on `gh_release` / `gh_release_rc` / `slim` / `sticky-*` |
...
| `controllers` | `DisplayController` plus sibling profiles selected at runtime (e.g. `XTEINK_X3_UC8279`) |
...
- [ ] Used committed `platformio.ini` ∩ CI, not `platformio.local.ini`, as the
      compile set.
- [ ] Loaded one resource per device flag on that set (`default` → x3 and x4).
