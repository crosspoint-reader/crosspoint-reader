...
- `_order` — Historical metadata, still validated for uniqueness but no longer used to compute the `Language` enum order (see `_bcp47` below).
...
- `_iso639_2` — The language's [ISO 639-2](https://www.loc.gov/standards/iso639-2/php/code_list.php) three-letter code(s), separated by spaces. Where there are two, list both (e.g. `"ger deu"`, `"fre fra"`). Required whenever `_bcp47` starts with a two-letter code: some books tag their language with the three-letter code, and the Library matches both.
- `_articles` — Optional. The leading articles the Library skips when sorting titles in this language, separated by spaces (e.g. `"the a an"`, `"le la les l' un une du des"`). Write each one lowercase and without accents, the way titles are compared; an elided article ends in an apostrophe (`l'`). A book is sorted with the articles of its own `dc:language`. A book whose language is missing or matches no translation file uses those of the UI language. Leave the key out for a language without articles, or whose words would be mistaken for them. Changing it re-sorts the Library the next time it opens.
