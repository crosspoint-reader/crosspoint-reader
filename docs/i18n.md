...
- `_bcp47` — [BCP 47](https://www.rfc-editor.org/rfc/rfc5646) language tag (e.g. `"pt-PT"`, `"ca-valencia"`). Must be unique. Controls the position in the `Language` enum (English is always 0, then the rest sorted alphabetically by this tag). Has no effect on the visible Settings menu order, which is sorted by `_language_name` instead. Persisted user language preferences are stored by `_language_code` string, not by the enum ordinal, so changing `_bcp47` (and thus the enum order) does not reset anyone's saved language.
