...
1. Copy your dictionary folder(s) to `/dictionaries/` on the SD card.
...
The command produces (when applicable) `.dict`, `.syn`, `.idx.fpi`, and `.syn.fpi`. The `.idx.fpi`/`.syn.fpi` files are accelerator indexes used at lookup time; if you copy a dictionary folder by hand, include them so the device skips on-device generation. The device will regenerate any that are missing on first use.
