...
- **Hyphenation**: Whether to hyphenate text in Reading Mode; options are "ON" or "OFF". Korean text wraps only at spaces when "OFF"; when "ON", a Korean word of four or more syllables may also wrap between syllables, keeping at least two on each line (no hyphen is drawn).
