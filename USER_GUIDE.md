...
The UI includes Arabic and Hebrew (menus use built-in fonts with presentation-form coverage). Built-in **reader** fonts do not cover Chinese, Japanese, Korean, Arabic, Bengali, Greek, Hebrew, or Farsi for book text. **CJK, Hebrew, Arabic, Bengali, Greek, and other extended scripts can be enabled for reading by installing custom SD card fonts** — see [Custom Fonts (SD Card)](#38-custom-fonts-sd-card). Bengali text is fully shaped (conjuncts, reph and vowel-sign placement) with the `NotoSansBengali` or `NotoSerifBengali` download fonts; see [docs/sd-card-fonts.md](docs/sd-card-fonts.md#bengali-bangla).
