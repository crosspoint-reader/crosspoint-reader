...
uint8_t CrossPointSettings::writeSettings(FsFile& file, bool count_only) const {
  ...
  writer.writeItemString(file, koReaderPageTurnerAddress);
...
bool CrossPointSettings::loadFromFile() {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    {
      ...
      strncpy(koReaderPageTurnerAddress, addrStr.c_str(), sizeof(koReaderPageTurnerAddress) - 1);
      koReaderPageTurnerAddress[sizeof(koReaderPageTurnerAddress) - 1] = '\0';
    }
    if (++settingsRead >= fileSettingsCount) break;
