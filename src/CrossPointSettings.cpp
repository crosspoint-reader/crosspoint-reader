...
bool CrossPointSettings::loadFromFile() {
  ...
  do {
    ...
    {
      std::string langStr;
      serialization::readString(inputFile, langStr);
      if (langStr.length() >= 2 && langStr.length() < sizeof(languageCode)) {
        strncpy(languageCode, langStr.c_str(), sizeof(languageCode) - 1);
