...
bool CrossPointSettings::saveToFile() const {
  ...
  serialization::writeString(outputFile, std::string(sleepScreenImagePath));
...
bool CrossPointSettings::loadFromFile() {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    {
      ...
      serialization::readString(inputFile, imagePathStr);
      strncpy(sleepScreenImagePath, imagePathStr.c_str(), sizeof(sleepScreenImagePath) - 1);
      sleepScreenImagePath[sizeof(sleepScreenImagePath) - 1] = '\0';
    }
    if (++settingsRead >= fileSettingsCount) break;
    // New fields added at end for backward compatibility
  } while (false);
