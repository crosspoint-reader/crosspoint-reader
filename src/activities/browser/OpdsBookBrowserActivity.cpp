...
void OpdsBookBrowserActivity::loop() {
  if (state == BrowserState::WIFI_SELECTION || state == BrowserState::DOWNLOADING) {
    return;
  }
...
void OpdsBookBrowserActivity::render(RenderLock&&) {
  ...
  if (state == BrowserState::DOWNLOADING) {
    ...
    const auto title = renderer.truncatedText(UI_10_FONT_ID, statusMessage.c_str(), maxWidth);
...
void OpdsBookBrowserActivity::fetchFeed(const std::string& path) {
  ...
  const std::string url = UrlUtils::buildUrl(serverUrl, path);
...
void OpdsBookBrowserActivity::downloadBook(const OpdsEntry& book) {
  ...
  const std::string downloadUrl = UrlUtils::buildUrl(SETTINGS.opdsServerUrl, book.href);

  std::string baseName = book.title;
  if (!book.author.empty()) {
    baseName += " - " + book.author;
  }
  const std::string filename = "/" + StringUtils::sanitizeFilename(baseName) + ".epub";
