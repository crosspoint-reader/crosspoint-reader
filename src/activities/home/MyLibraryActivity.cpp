...
void MyLibraryActivity::render() const {
  ...
  auto folderName = basepath == "/" ? T("SD card") : basepath.substr(basepath.rfind('/') + 1).c_str();
  ...
  if (files.empty()) {
    renderer.drawText(UI_10_FONT_ID, metrics.contentSidePadding, contentTop + 20, T("No books found"));
  } else {
    GUI.drawList(
