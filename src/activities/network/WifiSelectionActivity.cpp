...
void WifiSelectionActivity::selectNetwork(const int index) {
  ...
  if (selectedRequiresPassword) {
    ...
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, i18n(ENTER_WIFI_PASSWORD),
        "",     // No initial text
        50,     // Y position
...
void WifiSelectionActivity::checkConnectionStatus() {
  ...
  if (status == WL_CONNECT_FAILED || status == WL_NO_SSID_AVAIL) {
    connectionError = i18n(ERROR_GENERAL_FAILURE);
    if (status == WL_NO_SSID_AVAIL) {
      connectionError = i18n(ERROR_NETWORK_NOT_FOUND);
    }
  ...
  if (millis() - connectionStartTime > CONNECTION_TIMEOUT_MS) {
    WiFi.disconnect();
    connectionError = i18n(ERROR_CONNECTION_TIMEOUT);
    state = WifiSelectionState::CONNECTION_FAILED;
    updateRequired = true;
    return;
  }
...
void WifiSelectionActivity::renderNetworkList() const {
  ...
  // Draw header
  renderer.drawCenteredText(UI_12_FONT_ID, 15, i18n(WIFI_NETWORKS), true, EpdFontFamily::BOLD);

  if (networks.empty()) {
    // No networks found or scan failed
    const auto height = renderer.getLineHeight(UI_10_FONT_ID);
    const auto top = (pageHeight - height) / 2;
    renderer.drawCenteredText(UI_10_FONT_ID, top, i18n(NO_NETWORKS));
    renderer.drawCenteredText(SMALL_FONT_ID, top + height + 10, i18n(PRESS_OK_SCAN));
  } else {
    ...
    // Show network count
    char countStr[64];
    snprintf(countStr, sizeof(countStr), i18n(NETWORKS_FOUND), networks.size());
  ...
  // Draw help text
  renderer.drawText(SMALL_FONT_ID, 20, pageHeight - 75, i18n(NETWORK_LEGEND));
  const auto labels = mappedInput.mapLabels(i18n(BACK), i18n(CONNECT), "", "");
...
void WifiSelectionActivity::renderConnecting() const {
  ...
  if (state == WifiSelectionState::SCANNING) {
    renderer.drawCenteredText(UI_10_FONT_ID, top, i18n(SCANNING));
  } else {
    renderer.drawCenteredText(UI_12_FONT_ID, top - 40, i18n(CONNECTING), true, EpdFontFamily::BOLD);

    std::string ssidInfo = std::string(i18n(TO_PREFIX)) + selectedSSID;
...
void WifiSelectionActivity::renderConnected() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 30, i18n(CONNECTED), true, EpdFontFamily::BOLD);

  std::string ssidInfo = std::string(i18n(NETWORK_PREFIX)) + selectedSSID;
  if (ssidInfo.length() > 28) {
    ssidInfo.replace(25, ssidInfo.length() - 25, "...");
  }
  renderer.drawCenteredText(UI_10_FONT_ID, top + 10, ssidInfo.c_str());

  const std::string ipInfo = std::string(i18n(IP_ADDRESS_PREFIX)) + connectedIP;
...
void WifiSelectionActivity::renderSavePrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 40, i18n(CONNECTED), true, EpdFontFamily::BOLD);
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, i18n(SAVE_PASSWORD));
  ...
  // Draw "Yes" button
  if (savePromptSelection == 0) {
    std::string text = "[" + std::string(i18n(YES)) + "]";
    renderer.drawText(UI_10_FONT_ID, startX, buttonY, text.c_str());
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + 4, buttonY, i18n(YES));
  }

  // Draw "No" button
  if (savePromptSelection == 1) {
    std::string text = "[" + std::string(i18n(NO)) + "]";
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing, buttonY, text.c_str());
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing + 4, buttonY, i18n(NO));
  }

  // Use centralized button hints
  const auto labels = mappedInput.mapLabels(i18n(CANCEL), i18n(SELECT), i18n(DIR_LEFT), i18n(DIR_RIGHT));
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderConnectionFailed() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 20, i18n(CONNECTION_FAILED), true, EpdFontFamily::BOLD);
  renderer.drawCenteredText(UI_10_FONT_ID, top + 20, connectionError.c_str());

  // Use centralized button hints
  const auto labels = mappedInput.mapLabels(i18n(BACK), i18n(DONE), "", "");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderForgetPrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 40, i18n(CONNECTION_FAILED), true, EpdFontFamily::BOLD);
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, i18n(FORGET_AND_REMOVE));
  ...
  // Draw "Cancel" button
  if (forgetPromptSelection == 0) {
    std::string text = "[" + std::string(i18n(CANCEL)) + "]";
    renderer.drawText(UI_10_FONT_ID, startX, buttonY, text.c_str());
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + 4, buttonY, i18n(CANCEL));
  }

  // Draw "Forget network" button
  if (forgetPromptSelection == 1) {
    std::string text = "[" + std::string(i18n(FORGET_BUTTON)) + "]";
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing, buttonY, text.c_str());
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing + 4, buttonY, i18n(FORGET_BUTTON));
  }
