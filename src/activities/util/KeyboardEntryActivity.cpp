...
void KeyboardEntryActivity::render(RenderLock&&) {
  ...
  if (hintVisible && !text.empty()) {
    ...
    // Same landscape fit problem as the tips block below: keyboardRect() sizes
    // keys in fixed pixels regardless of orientation, so a short landscape
    // screen can leave no room above the keyboard for these lines either.
    const int linesNeeded = cursorMode ? (inputType == InputType::Password ? 2 : 1) : 1;
    if (kbRect.y - hintY >= linesNeeded * hintLh) {
      if (cursorMode) {
        int hintLineY = hintY;
