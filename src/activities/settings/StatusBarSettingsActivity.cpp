...
enum MenuItem {
  ...
  ITEM_CLOCK_TIME_ZONE,  // X3 only, launches ClockTimezoneActivity
...
void StatusBarSettingsActivity::handleSelection() {
  switch (nav.selected) {
    ...
    case ITEM_CLOCK_FORMAT:
      ...
      break;
    case ITEM_CLOCK_TIME_ZONE: {
      auto activity = makeUniqueNoThrow<ClockTimezoneActivity>(renderer, mappedInput);
      if (!activity) {
        LOG_ERR("STATUS", "OOM: ClockTimezoneActivity");
        return;
      }
...
std::string StatusBarSettingsActivity::rowValueText(const int index) {
  switch (index) {
    ...
    case ITEM_CLOCK_TIME_ZONE:
      return I18N.get(getClockTimeZone(SETTINGS.clockTimeZone).label);
    case ITEM_CLOCK_SYNC:
