...
const char* menuKeys[MENU_ITEMS] = {"OPDS Server URL", "Username", "Password"};
}  // namespace
...
void CalibreSettingsActivity::handleSelection() {
  ...
  if (selectedIndex == 0) {
    ...
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, T("OPDS Server URL"), SETTINGS.opdsServerUrl, 10,
        127,    // maxLength
        false,  // not password
    ...
    exitActivity();
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, T("Username"), SETTINGS.opdsUsername, 10,
        63,     // maxLength
        false,  // not password
    ...
    exitActivity();
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, T("Password"), SETTINGS.opdsPassword, 10,
        63,     // maxLength
...
void CalibreSettingsActivity::render() {
  ...
  // Draw header
  renderer.drawCenteredText(UI_12_FONT_ID, 15, T("OPDS Browser"), true, EpdFontFamily::BOLD);

  // Draw info text about Calibre
  renderer.drawCenteredText(UI_10_FONT_ID, 40, T("For Calibre, add /opds to your URL"));
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    const char* status = T("[Not Set]");
    if (i == 0) {
      status = (strlen(SETTINGS.opdsServerUrl) > 0) ? T("[Set]") : T("[Not Set]");
    } else if (i == 1) {
      status = (strlen(SETTINGS.opdsUsername) > 0) ? T("[Set]") : T("[Not Set]");
    } else if (i == 2) {
      status = (strlen(SETTINGS.opdsPassword) > 0) ? T("[Set]") : T("[Not Set]");
    }
