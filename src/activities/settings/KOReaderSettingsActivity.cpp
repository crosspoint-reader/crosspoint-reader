...
const StrId menuNames[MENU_ITEMS] = {StrId::STR_USERNAME,
                                     ...
                                     StrId::STR_ALLOW_UNTRUSTED_CERT,
...
void KOReaderSettingsActivity::handleSelection() {
  if (selectedIndex == 0) {
    ...
    // Allow untrusted certificate - toggle
    KOREADER_STORE.setAllowUntrustedCert(!KOREADER_STORE.getAllowUntrustedCert());
    KOREADER_STORE.saveToFile();
    requestUpdate();
  ...
  GUI.drawList(
      ...
      [this](int index) {
        ...
        if (index == 0) {
          ...
          return std::string(KOREADER_STORE.getAllowUntrustedCert() ? tr(STR_YES) : tr(STR_NO));
        } else if (index == 5) {
