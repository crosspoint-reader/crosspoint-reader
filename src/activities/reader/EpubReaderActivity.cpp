...
void EpubReaderActivity::openReaderMenu() {
  ...
  startActivityForResult(
      std::make_unique<EpubReaderMenuActivity>(
          ...
          autoPageTurnSeconds, automaticPageTurnActive, !currentPageFootnotes.empty(), !cachedBookmarks.empty(),
...
void EpubReaderActivity::loop() {
  ...
  if (safeModePromptRequested.exchange(false, std::memory_order_acq_rel)) {
    startActivityForResult(
        std::make_unique<ConfirmationActivity>(renderer, mappedInput, tr(STR_EPUB_SAFE_MODE),
  ...
  if (SETTINGS.longPressMenuFunction != CrossPointSettings::LP_MENU_DISABLED &&
    ...
    switch (SETTINGS.longPressMenuFunction) {
      ...
      case CrossPointSettings::LP_MENU_AUTO_PAGE_TURN:
        applyAutoPageTurnRuntime(ReaderUtils::autoPageTurnShortcutSeconds(autoPageTurnSeconds),
...
void EpubReaderActivity::openClippingSelection() {
  ...
  startActivityForResult(std::make_unique<ClipSelectionActivity>(renderer, mappedInput, std::move(page),
                                                                 SETTINGS.getReaderFontId(), marginLeft, marginTop,
                                                                 currentPage, pageCount, paragraphIndex, pageLoader),
                         [this](const ActivityResult& result) {
...
void EpubReaderActivity::armClippingJump(const ClippingJumpResult& jump) {
  ...
  pendingClippingJump = PendingClippingJump{
      jump.spineIndex, jump.startPage,   jump.pageFingerprint,        currentSpineIndex,
      fallbackPage,    cachedSpineIndex, cachedChapterTotalPageCount,
  };
  ...
  nextPageNumber = jump.startPage;
  if (keepSection) {
    section->currentPage = jump.startPage;
  } else {
    section.reset();
  }
}

bool EpubReaderActivity::abortPendingClippingJump() {
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  if (!section) {
    ...
    const bool cacheLoaded = section->loadSectionFile(
        ...
        activeEmbeddedStyle(), SETTINGS.imageRendering, SETTINGS.focusReadingEnabled, activeEpubRenderMode(),
        SETTINGS.forceParagraphIndents != 0);
    if (cacheLoaded) {
...

  {
    ...
    if (pendingClippingJump) {
      ...
      if (!exactTarget) {
        LOG_ERR("ERS", "Exact clipping target fingerprint mismatch - restoring previous position");
        automaticPageTurnActive = false;
        abortPendingClippingJump();
        signalReadingPageHidden();
        showPendingSyncSaveError();
...
bool EpubReaderActivity::renderContents(std::unique_ptr<Page> page, const int orientedMarginTop,
  ...
  if (needsAnyGrayscale && renderer.supportsStripGrayscale()) {
    ...
    if (needsAnyGrayscale) {
      ...
      EPUB_RENDER_TIMESTAMP(tBwStore);
      ...
      EPUB_RENDER_TIMESTAMP(tBwRestore);
      ...
      LOG_DBG("ERS",
              ...
              static_cast<unsigned>(tPrewarm - t0), static_cast<unsigned>(tBwRender - tPrewarm),
              static_cast<unsigned>(tDisplay - tBwRender), static_cast<unsigned>(tBwStore - tDisplay),
              static_cast<unsigned>(tGrayLsb - tBwStore), static_cast<unsigned>(tGrayMsb - tGrayLsb),
              static_cast<unsigned>(tGrayDisplay - tGrayMsb), static_cast<unsigned>(tBwRestore - tGrayDisplay),
              static_cast<unsigned>(tEnd - t0));
    } else {
...
void EpubReaderActivity::loadCachedBookmarks() {
  ...
  const JsonSettingsIO::BookmarkLoadStatus loaded = JsonSettingsIO::loadBookmarksFromFile(cachedBookmarks, bmPath.c_str());
  if (loaded == JsonSettingsIO::BookmarkLoadStatus::Loaded ||
      loaded == JsonSettingsIO::BookmarkLoadStatus::Missing) {
    bookmarksWritable = true;
  } else {
    cachedBookmarks.clear();
