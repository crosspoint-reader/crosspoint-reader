...
void EpubReaderActivity::loop() {
  ...
  // Short press BACK goes directly to home (or cancels capture / restores footnote position)
  ...
  if (prevTriggered) {
    ...
    if (captureState == CaptureState::CAPTURING) {
      if (section) {
        ...
        // Section boundary crossed — defer capture until render() loads the new section
      ...
      startActivityForResult(
          ...
          [this](const ActivityResult& result) {
            if (!result.isCancelled) {
              ...
              if (currentSpineIndex != bk.spineIndex || (section && section->currentPage != bk.pageIndex)) {
                currentSpineIndex = bk.spineIndex;
                nextPageNumber = bk.pageIndex;
                section.reset();
              }
            }
          });
...
void EpubReaderActivity::renderStatusBar() const {
  // Temporary status bar override (e.g. "Passage saved", "Bookmarked")
