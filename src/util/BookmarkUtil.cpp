    ...
    for (int i = 0; i < maxBookmarks; i++) {
      ...
      if (Storage.openFileForRead("BKM", epub->getCachePath() + "/bookmark_" + std::to_string(i) + ".bin", f)) {
        ...
        if (dataSize == 4 || dataSize == 6) {
          ...
          LOG_DBG("BKM", "Loaded bookmark: %d, %d/%d with summary: %s", currentSpineIndex, pageNumber, pageCount, summary.c_str());
...
BookmarkItem BookmarkUtil::saveBookmark(int bookmarkIndex, int currentSpineIndex, int currentPage, int pageCount, std::string summary) {
  FsFile f;
  if (Storage.openFileForWrite("BKM", epub->getCachePath() + "/bookmark_" + std::to_string(bookmarkIndex) + ".bin", f)) {
    ...
    LOG_DBG("BKM", "Bookmark saved: Chapter %d, Page %d with summary: %s", currentSpineIndex, currentPage, summary.c_str());
  } else {
