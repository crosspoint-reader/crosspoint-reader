...
void BaseTheme::drawProgressBar(const GfxRenderer& renderer, Rect rect, const size_t current,
  ...
  renderer.drawCenteredText(PULSR_10_FONT_ID, rect.y + rect.height + 15, percentText.c_str());
}
...
void BaseTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (labels[i] != nullptr && labels[i][0] != '\0') {
      ...
      const int textWidth = renderer.getTextWidth(PULSR_10_FONT_ID, labels[i]);
      const int textX = x + (buttonWidth - 1 - textWidth) / 2;
      renderer.drawText(PULSR_10_FONT_ID, textX, pageHeight - buttonY + textYOffset, labels[i]);
    }
  }
...
void BaseTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    auto font = (rowSubtitle != nullptr) ? PULSR_12_FONT_ID : PULSR_10_FONT_ID;
    ...
    if (rowSubtitle != nullptr) {
      ...
      auto subtitle = renderer.truncatedText(PULSR_10_FONT_ID, subtitleText.c_str(), textWidth);
      renderer.drawText(PULSR_10_FONT_ID, rect.x + BaseMetrics::values.contentSidePadding, itemY + 30, subtitle.c_str(),
                        i != selectedIndex);
    }
...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  if (title) {
    ...
    auto truncatedTitle = renderer.truncatedText(PULSR_12_FONT_ID, title,
                                                 rect.width - padding * 2 - BaseMetrics::values.contentSidePadding * 2,
                                                 EpdFontFamily::BOLD);
    renderer.drawCenteredText(PULSR_12_FONT_ID, rect.y + 5, truncatedTitle.c_str(), true, EpdFontFamily::BOLD);
  }
...
void BaseTheme::drawTabBar(const GfxRenderer& renderer, const Rect rect, const std::vector<TabInfo>& tabs,
  ...
  for (const auto& tab : tabs) {
    const int textWidth =
        renderer.getTextWidth(PULSR_12_FONT_ID, tab.label, tab.selected ? EpdFontFamily::BOLD : EpdFontFamily::REGULAR);
    ...
    // Draw tab label
    renderer.drawText(PULSR_12_FONT_ID, currentX, rect.y, tab.label, !(tab.selected && selected),
...
void BaseTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    const int spaceWidth = renderer.getSpaceWidth(PULSR_12_FONT_ID);
    ...
    for (auto& i : words) {
      ...
      if (lines.size() >= 3) {
        ...
        while (!lines.back().empty() && renderer.getTextWidth(PULSR_12_FONT_ID, lines.back().c_str()) > maxLineWidth) {
      ...
      int wordWidth = renderer.getTextWidth(PULSR_12_FONT_ID, i.c_str());
      while (wordWidth > maxLineWidth && !i.empty()) {
        ...
        wordWidth = renderer.getTextWidth(PULSR_12_FONT_ID, withEllipsis.c_str());
      ...
      int newLineWidth = renderer.getTextWidth(PULSR_12_FONT_ID, currentLine.c_str());
      if (newLineWidth > 0) {
    ...
    // Book title text
    int totalTextHeight = renderer.getLineHeight(PULSR_12_FONT_ID) * static_cast<int>(lines.size());
    if (!lastBookAuthor.empty()) {
      totalTextHeight += renderer.getLineHeight(PULSR_10_FONT_ID) * 3 / 2;
    }
    ...
    if (coverRendered) {
      ...
      for (const auto& line : lines) {
        const int lineWidth = renderer.getTextWidth(PULSR_12_FONT_ID, line.c_str());
      ...
      if (!lastBookAuthor.empty()) {
        ...
        while (renderer.getTextWidth(PULSR_10_FONT_ID, trimmedAuthor.c_str()) > maxLineWidth && !trimmedAuthor.empty()) {
          utf8RemoveLastChar(trimmedAuthor);
        }
        if (renderer.getTextWidth(PULSR_10_FONT_ID, trimmedAuthor.c_str()) <
            renderer.getTextWidth(PULSR_10_FONT_ID, lastBookAuthor.c_str())) {
          trimmedAuthor.append("...");
        }
        const int authorWidth = renderer.getTextWidth(PULSR_10_FONT_ID, trimmedAuthor.c_str());
    ...
    for (const auto& line : lines) {
      renderer.drawCenteredText(PULSR_12_FONT_ID, titleYStart, line.c_str(), !bookSelected);
      titleYStart += renderer.getLineHeight(PULSR_12_FONT_ID);
    }
    ...
    if (!lastBookAuthor.empty()) {
      titleYStart += renderer.getLineHeight(PULSR_10_FONT_ID) / 2;
      std::string trimmedAuthor = lastBookAuthor;
      ...
      if (wasTrimmed && !trimmedAuthor.empty()) {
        ...
        while (renderer.getTextWidth(PULSR_10_FONT_ID, (trimmedAuthor + "...").c_str()) > maxLineWidth &&
               !trimmedAuthor.empty()) {
          utf8RemoveLastChar(trimmedAuthor);
        }
        trimmedAuthor.append("...");
      }
      renderer.drawCenteredText(PULSR_10_FONT_ID, titleYStart, trimmedAuthor.c_str(), !bookSelected);
    }

    // "Continue Reading" label at the bottom
    const int continueY = bookY + bookHeight - renderer.getLineHeight(PULSR_10_FONT_ID) * 3 / 2;
    if (coverRendered) {
      // Draw box behind "Continue Reading" text (inverted when selected: black box instead of white)
      const char* continueText = tr(STR_CONTINUE_READING);
      const int continueTextWidth = renderer.getTextWidth(PULSR_10_FONT_ID, continueText);
      constexpr int continuePadding = 6;
      const int continueBoxWidth = continueTextWidth + continuePadding * 2;
      const int continueBoxHeight = renderer.getLineHeight(UI_10_FONT_ID) + continuePadding;
      const int continueBoxX = rect.x + (rect.width - continueBoxWidth) / 2;
      const int continueBoxHeight = renderer.getLineHeight(PULSR_10_FONT_ID) + continuePadding;
      const int continueBoxX = (rect.width - continueBoxWidth) / 2;
      const int continueBoxY = continueY - continuePadding / 2;
      renderer.fillRect(continueBoxX, continueBoxY, continueBoxWidth, continueBoxHeight, bookSelected);
      renderer.drawRect(continueBoxX, continueBoxY, continueBoxWidth, continueBoxHeight, !bookSelected);
      renderer.drawCenteredText(PULSR_10_FONT_ID, continueY, continueText, !bookSelected);
    } else {
      renderer.drawCenteredText(PULSR_10_FONT_ID, continueY, tr(STR_CONTINUE_READING), !bookSelected);
    }
  } else {
    // No book to continue reading
    const int y =
        bookY + (bookHeight - renderer.getLineHeight(PULSR_12_FONT_ID) - renderer.getLineHeight(PULSR_10_FONT_ID)) / 2;
    renderer.drawCenteredText(PULSR_12_FONT_ID, y, "No open book");
    renderer.drawCenteredText(PULSR_10_FONT_ID, y + renderer.getLineHeight(PULSR_12_FONT_ID), "Start reading below");
  }
}
...
void BaseTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = 0; i < buttonCount; ++i) {
    ...
    const int textWidth = renderer.getTextWidth(PULSR_10_FONT_ID, label);
    const int textX = rect.x + (rect.width - textWidth) / 2;
    const int lineHeight = renderer.getLineHeight(PULSR_10_FONT_ID);
    const int textY =
        tileY + (BaseMetrics::values.menuRowHeight - lineHeight) / 2;  // vertically centered assuming y is top of text
    // Invert text when the tile is selected, to contrast with the filled background
    renderer.drawText(PULSR_10_FONT_ID, textX, textY, label, selectedIndex != i);
  }
}
...
Rect BaseTheme::drawPopup(const GfxRenderer& renderer, const char* message) const {
  ...
  constexpr int y = 60;
  const int textWidth = renderer.getTextWidth(PULSR_12_FONT_ID, message, EpdFontFamily::BOLD);
  const int textHeight = renderer.getLineHeight(PULSR_12_FONT_ID);
  ...
  renderer.drawText(PULSR_12_FONT_ID, textX, textY, message, true, EpdFontFamily::BOLD);
...
void BaseTheme::drawTextField(const GfxRenderer& renderer, Rect rect, const int textWidth) const {
  renderer.drawText(PULSR_12_FONT_ID, rect.x + 10, rect.y, "[");
  renderer.drawText(PULSR_12_FONT_ID, rect.x + rect.width - 15, rect.y + rect.height, "]");
}

void BaseTheme::drawKeyboardKey(const GfxRenderer& renderer, Rect rect, const char* label,
                                const bool isSelected) const {
  const int itemWidth = renderer.getTextWidth(PULSR_10_FONT_ID, label);
  const int textX = rect.x + (rect.width - itemWidth) / 2;
  if (isSelected) {
    renderer.drawText(PULSR_10_FONT_ID, textX - 6, rect.y, "[");
    renderer.drawText(PULSR_10_FONT_ID, textX + itemWidth, rect.y, "]");
  }
  renderer.drawText(PULSR_10_FONT_ID, textX, rect.y, label);
}
