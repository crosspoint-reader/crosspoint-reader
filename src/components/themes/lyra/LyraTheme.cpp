...
void LyraTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  if (list_ != nullptr && list_->enabled) {
    ...
    for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
      ...
      const bool underlineTitle =
          selected && spec.selectionStyle == ThemeMenuSelectionStyle::Underline && subtitleText.empty();
      if (underlineTitle) {
      ...
      if (rowSubtitle != nullptr && !subtitleText.empty()) {
        ...
        if (selected && spec.selectionStyle == ThemeMenuSelectionStyle::Underline) {
          const int underlineWidth = renderer.getTextWidth(spec.subtitleFontId, subtitle.c_str());
          const int underlineY =
              std::min(itemY + rowHeight - 4, subtitleY + renderer.getLineHeight(spec.subtitleFontId) + 2);
          renderer.drawLine(textX, underlineY, textX + underlineWidth - 1, underlineY, 1, true);
        }
      }
...
void LyraTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (homeRecents_ != nullptr && homeRecents_->type == ThemeHomeRecentsType::CoverStrip) {
    drawCoverStripRecents(renderer, rect, recentBooks, coverSelectorIndex, coverRendered, coverBufferStored,
                          bufferRestored, storeCoverBuffer, coverStripSelected);
    return;
  }
  ...
  if (hasContinueReading) {
    ...
    bool bookSelected = coverStripSelected;
...
void LyraTheme::drawCoverStripRecents(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  auto drawCover = [&](int bookIndex, int x, int y, int w, int h, bool selectedCover) {
    ...
    const int lineWidth = selectedCover ? std::max(1, coverStripSelected ? spec.selectionLineWidth : inactiveWidth) : 0;
    const int clearPad = selectedCover ? 6 + std::max(0, lineWidth - 1) : 0;
    renderer.fillRect(x - clearPad, y - clearPad, w + clearPad * 2, h + clearPad * 2, false);
  ...
  auto drawSlot = [&](const ThemeCoverSlotSpec& slot) {
