...
void CrossPointWebServer::handleDelete() const {
  // Support legacy single-path API: ?path=/foo.epub → wrap as ["\/foo.epub"]
  // New API uses ?paths=["\/foo.epub","\/bar.epub"] for batch deletes.
  ...
  if (server->hasArg("paths")) {
    ...
    // Wrap single path into a 1-element JSON array
    String singlePath = server->arg("path");
    singlePath.replace("\"", "\\\"");
    pathsArg = "[\"" + singlePath + "\"]";
  }
  JsonDocument doc;
