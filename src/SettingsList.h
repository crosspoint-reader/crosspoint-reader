...
//
// Uses push_back rather than initializer_list to avoid allocating all 39 SettingInfo
// temporaries on the stack simultaneously (would overflow the 8KB loopTask stack).
inline std::vector<SettingInfo> getSettingsList() {
  std::vector<SettingInfo> list;
  list.reserve(39);

  // --- Display ---
  list.push_back(SettingInfo::Enum(StrId::STR_SLEEP_SCREEN, &CrossPointSettings::sleepScreen,
  ...
  list.push_back(SettingInfo::Toggle(StrId::STR_LONG_PRESS_SKIP, &CrossPointSettings::longPressChapterSkip,
  ...
  // --- Feed Sync (web-only) ---
  list.push_back(SettingInfo::String(StrId::STR_FEED_URL, SETTINGS.feedUrl, sizeof(SETTINGS.feedUrl), "feedUrl",
                                     StrId::STR_CAT_FEED_SYNC));
  list.push_back(SettingInfo::Value(StrId::STR_FEED_NEWS_DAYS, &CrossPointSettings::feedNewsDays, {1, 30, 1},
                                    "feedNewsDays", StrId::STR_CAT_FEED_SYNC));
  list.push_back(SettingInfo::Toggle(StrId::STR_FEED_ALLOW_FIRMWARE, &CrossPointSettings::feedAllowFirmware,
                                     "feedAllowFirmware", StrId::STR_CAT_FEED_SYNC));

  // --- Danger Zone (device UI only; hidden from web settings API for security) ---
  list.push_back(SettingInfo::Toggle(StrId::STR_DANGER_ZONE, &CrossPointSettings::dangerZoneEnabled,
                                     "dangerZoneEnabled", StrId::STR_CAT_DANGER_ZONE)
                     .withDeviceOnly());
  list.push_back(SettingInfo::String(StrId::STR_DANGER_ZONE_PASSWORD, SETTINGS.dangerZonePassword,
                                     sizeof(SETTINGS.dangerZonePassword), "dangerZonePassword",
