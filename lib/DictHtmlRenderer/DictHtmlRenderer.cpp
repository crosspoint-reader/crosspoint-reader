...
void DictHtmlRenderer::recordUnknownTag(const char* tagName, const char* wordBefore, const char* tagContents,
  ...
  strlcpy(info.tag, tagName ? tagName : "", sizeof(info.tag));
  strlcpy(info.entry, currentEntryName ? currentEntryName : "", sizeof(info.entry));
  strlcpy(info.wordBefore, wordBefore ? wordBefore : "", sizeof(info.wordBefore));
  strlcpy(info.tagContents, tagContents ? tagContents : "", sizeof(info.tagContents));
  strlcpy(info.wordAfter, wordAfter ? wordAfter : "", sizeof(info.wordAfter));
}
