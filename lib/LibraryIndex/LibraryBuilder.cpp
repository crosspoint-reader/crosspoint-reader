...
[[gnu::noinline]] bool stageRecord(WalkState& st, const std::string& name, const uint32_t fileSize,
  ...
  if (reuseMetadata) {
    ...
    if (priorSeries.seriesId != CLIX_SERIES_NONE) {
      ...
      if (!st.previous->readSeries(priorSeries.seriesId, series, priorSeriesBooks)) {
        st.failed = true;
        return false;
      }
...
bool emitIndex(const char* folderStagePath, WalkState& st, const uint16_t* order, const uint16_t* resolvedFirstSeen,
  ...
  //
  // Gated on the same signal as the author sort: a library that cannot be
  // ordered by author cannot be ordered by series either, and this array is
  // 24 bytes a book against 14 for the others.
  //
  ...
  if (seriesOrderOf && seriesIdOf && seriesIndexOf && st.readMetadata && coreSortsAvailable) {
