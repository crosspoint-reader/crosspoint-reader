...
bool Fb2CoverExtractor::extract() const {
  ...
  FsFile coverJpg;
  if (!SdMan.openFileForRead("FB2", tempJpegPath, coverJpg)) {
    SdMan.remove(tempJpegPath.c_str());
    return false;
  }

  FsFile coverBmp;
  if (!SdMan.openFileForWrite("FB2", outputBmpPath, coverBmp)) {
    coverJpg.close();
    SdMan.remove(tempJpegPath.c_str());
    return false;
  }
...
bool Fb2CoverExtractor::extractThumb(const std::string& thumbPath, int height) const {
  ...
  FsFile thumbBmp;
  if (!SdMan.openFileForWrite("FB2", thumbPath, thumbBmp)) {
    coverJpg.close();
    SdMan.remove(tempJpegPath.c_str());
    return false;
  }
