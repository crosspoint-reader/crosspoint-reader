...
// ---------------------------------------------------------------------------
// linearScan — fallback: read .dict line by line, compare words
// ---------------------------------------------------------------------------
bool DictionaryIndex::linearScan(const char* dictPath, const char* word, char* outDef, int outDefSize) {
  if (outDefSize <= 0) return false;
  outDef[0] = '\0';
  ...
  static constexpr int SCAN_BUF_SIZE = 1280;
  auto* lineBuf = static_cast<char*>(malloc(SCAN_BUF_SIZE));
  if (!lineBuf) {
    LOG_ERR("DICT", "malloc failed for linearScan buffer");
    dictFile.close();
    return false;
  }

  while (true) {
    int len = readLine(dictFile, lineBuf, SCAN_BUF_SIZE);
    if (len == 0 && dictFile.available() <= 0) break;
    if (len == 0) continue;
    ...
    int wordLen = static_cast<int>(tab - lineBuf);
    char lineWord[256];
    if (wordLen >= static_cast<int>(sizeof(lineWord))) wordLen = static_cast<int>(sizeof(lineWord)) - 1;
    memcpy(lineWord, lineBuf, wordLen);
    lineWord[wordLen] = '\0';
    ...
    if (strcasecmp(word, lineWord) == 0) {
      ...
      if (defLen >= outDefSize) {
        memcpy(outDef, def, outDefSize - 4);
        memcpy(outDef + outDefSize - 4, "...", 4);
      } else {
        memcpy(outDef, def, defLen + 1);
      }
      dictFile.close();
      free(lineBuf);
      return true;
    ...
    // Since the file is sorted, if we've passed the word, stop early
    if (strcasecmp(lineWord, word) > 0) break;
  }
