...
void ParsedText::layoutAndExtractLines(const GfxRenderer& renderer, const int fontId, const uint16_t viewportWidth,
  ...
  std::vector<size_t> lineBreakIndices;
  if (dropCap_) {
    // Drop caps need a per-line-ordinal inset, which the DP breaker cannot take, so lay
    // these out line-by-line with the greedy breaker.
    lineBreakIndices =
  ...
  for (size_t i = 0; i < lineCount; ++i) {
    if (dropCap_ && i == 0 && !capText.empty()) {
      ...
      auto capProcess = [&](std::shared_ptr<TextBlock> block, uint32_t offset) {
