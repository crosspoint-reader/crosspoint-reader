...
namespace {
constexpr uint8_t BOOK_CACHE_VERSION = 11;  // v11: cache the optimizer's original KOReader document ID
...
bool BookMetadataCache::buildBookBin(const std::string& epubPath, const BookMetadata& metadata) {
  ...
  const uint32_t metadataSize = metadata.title.size() + metadata.author.size() + metadata.language.size() +
                                ...
                                metadata.originalDocumentId.size() + sizeof(uint32_t) * 6;
  ...
  serialization::writeString(bookOut, metadata.originalDocumentId);
...
bool BookMetadataCache::load() {
  ...
  serialization::readString(bookFile, coreMetadata.originalDocumentId);
