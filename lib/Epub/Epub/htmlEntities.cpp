...
// REFACTORED TO LOCAL STATIC: Global maps of strings trigger massive heap allocations
// during static initialization. On ESP32, this can occur before the heap/OS is
// stable, leading to "Static Initialization Order Fiasco" and heap corruption.
// Moving this inside the function ensures it's initialized safely on first use.
...
bool process_string_entity(const std::string& entity, std::string& res) {
  static const std::unordered_map<std::string, std::string> entity_lookup(
      {{"&quot;", "\""},  {"&frasl;", "⁄"},   {"&amp;", "&"},      {"&lt;", "<"},     {"&gt;", ">"},
