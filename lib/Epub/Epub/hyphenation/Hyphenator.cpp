...
// Maps an alternate language tag to the canonical two-letter code used by the hyphenation
// registry.  EPUBs may use various forms in their dc:language metadata, so this covers more
// than ISO 639-2 normalization: both the bibliographic ("fre"/"ger") and terminological
// ("fra"/"deu") ISO 639-2 variants map to ISO 639-1, and Norwegian's BCP-47/639-1 tags
// (Bokmål "nb"/"nob", Nynorsk "nn"/"nno", and the macrolanguage code "nor") all map to the
// single combined Norwegian trie registered under "no".
struct LanguageTagAlias {
  const char* alternateTag;
  const char* canonicalTag;
};
static constexpr LanguageTagAlias kLanguageTagAliases[] = {{"eng", "en"}, {"fra", "fr"}, {"fre", "fr"}, {"deu", "de"},
                                                           {"ger", "de"}, {"rus", "ru"}, {"spa", "es"}, {"ita", "it"},
                                                           {"ukr", "uk"}, {"swe", "sv"}, {"nb", "no"},  {"nob", "no"},
                                                           {"nn", "no"},  {"nno", "no"}, {"nor", "no"}};
...
const LanguageHyphenator* hyphenatorForLanguage(const std::string& langTag) {
  ...
  // Normalize alternate language tags to their canonical two-letter registry equivalents.
  for (const auto& alias : kLanguageTagAliases) {
    if (primary == alias.alternateTag) {
      primary = alias.canonicalTag;
      break;
    }
  }
