...
void XMLCALL ChapterHtmlSlimParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (strcmp(name, "p") == 0) {
    self->typesetter.setXpathParagraphIndex(self->typesetter.getXpathParagraphIndex() + 1);
  }
  if (strcmp(name, "li") == 0) {
    self->typesetter.setXpathListItemIndex(self->typesetter.getXpathListItemIndex() + 1);
  }
...
void XMLCALL ChapterHtmlSlimParser::characterData(void* userData, const XML_Char* s, const int len) {
  ...
  if (self->currentTextBlock->size() > 750) {
    ...
    self->currentTextBlock->layoutAndExtractLines(
        ...
        [self](const std::shared_ptr<TextBlock>& textBlock) { self->typesetter.addLineToPage(textBlock); }, false);
  }
}
