...
bool Epub::parseContentOpf(BookMetadataCache::BookMetadata& bookMetadata) {
  ...
  LOG_INF("EBP", "Parsing content.opf: %s", contentOpfFilePath.c_str());
  ...
  LOG_INF("EBP", "Successfully parsed content.opf");
  return true;
}

bool Epub::parseTocNcxFile() const {
  if (tocNcxItem.empty()) {
    LOG_ERR("EBP", "No ncx file specified");
    return false;
  }

  LOG_INF("EBP", "Parsing toc ncx file: %s", tocNcxItem.c_str());
  ...
  LOG_INF("EBP", "Parsed TOC items");
  return true;
}

bool Epub::parseTocNavFile() const {
  if (tocNavItem.empty()) {
    LOG_ERR("EBP", "No nav file specified");
    return false;
  }

  LOG_INF("EBP", "Parsing toc nav file: %s", tocNavItem.c_str());
  ...
  LOG_INF("EBP", "Parsed TOC nav items");
  return true;
}
...
bool Epub::loadCssRulesFromCache() const {
  ...
  if (Storage.openFileForRead("EBP", getCssRulesCache(), cssCacheFile)) {
    if (cssParser->loadFromCache(cssCacheFile)) {
      cssCacheFile.close();
      LOG_INF("EBP", "Loaded CSS rules from cache");
      return true;
    }
    cssCacheFile.close();
...
void Epub::parseCssFiles() const {
  if (cssFiles.empty()) {
    LOG_INF("EBP", "No CSS files to parse, but CssParser created for inline styles");
  }

  if (!cssParser->hasCache()) {
    for (const auto& cssPath : cssFiles) {
      LOG_INF("EBP", "Parsing CSS file: %s", cssPath.c_str());
    ...
    cssParser->clear();
    LOG_INF("EBP", "Loaded %zu CSS style rules from %zu files", cssParser->ruleCount(), cssFiles.size());
  }
}

bool Epub::load(const bool buildIfMissing, const bool skipLoadingCss) {
  LOG_INF("EBP", "Loading ePub: %s", filepath.c_str());
  ...
  if (bookMetadataCache->load()) {
    ...
    LOG_INF("EBP", "Loaded ePub: %s", filepath.c_str());
    return true;
  }
  ...
  LOG_INF("EBP", "Cache not found, building spine/TOC cache");
  setupCacheDir();
  ...
  if (!tocNavItem.empty()) {
    LOG_INF("EBP", "Attempting to parse EPUB 3 nav document");
    tocParsed = parseTocNavFile();
  }
  if (!tocParsed && !tocNcxItem.empty()) {
    LOG_INF("EBP", "Falling back to NCX TOC");
    tocParsed = parseTocNcxFile();
  }
  if (!tocParsed) {
    LOG_INF("EBP", "Warning: Could not parse any TOC format");
  }
...
bool Epub::clearCache() const {
  if (!Storage.exists(cachePath.c_str())) {
    LOG_INF("EPB", "Cache does not exist, no action needed");
    return true;
  }
  ...
  LOG_INF("EPB", "Cache cleared successfully");
  return true;
}
...
bool Epub::generateCoverBmp(bool cropped) const {
  ...
  if (coverImageHref.substr(coverImageHref.length() - 4) == ".jpg" ||
    ...
    LOG_INF("EBP", "Generating BMP from JPG cover image (%s mode)", cropped ? "cropped" : "fit");
    ...
    LOG_INF("EBP", "Generated BMP from JPG cover image, success: %s", success ? "yes" : "no");
    return success;
  } else {
    LOG_INF("EBP", "Cover image is not a JPG, skipping");
  }
  return false;
}
...
bool Epub::generateThumbBmp(int height) const {
  ...
  const auto coverImageHref = bookMetadataCache->coreMetadata.coverItemHref;
  if (coverImageHref.empty()) {
    LOG_INF("EBP", "No known cover image for thumbnail");
  } else if (coverImageHref.substr(coverImageHref.length() - 4) == ".jpg" ||
             coverImageHref.substr(coverImageHref.length() - 5) == ".jpeg") {
    LOG_INF("EBP", "Generating thumb BMP from JPG cover image");
    const auto coverJpgTempPath = getCachePath() + "/.cover.jpg";
    FsFile coverJpg;
    if (!Storage.openFileForWrite("EBP", coverJpgTempPath, coverJpg)) {
      return false;
    ...
    LOG_INF("EBP", "Generated thumb BMP from JPG cover image, success: %s", success ? "yes" : "no");
    return success;
  } else {
    LOG_INF("EBP", "Cover image is not a JPG, skipping thumbnail");
  }
...
uint8_t* Epub::readItemContentsToBytes(const std::string& itemHref, size_t* size, const bool trailingNullByte) const {
  if (itemHref.empty()) {
    LOG_ERR("EBP", "Failed to read item, empty href");
    return nullptr;
  }
  const std::string path = FsHelpers::normalisePath(itemHref);
  const auto content = ZipFile(filepath).readFileToMemory(path.c_str(), size, trailingNullByte);
  if (!content) {
    LOG_ERR("EBP", "Failed to read item %s", path.c_str());
    return nullptr;
  }
  return content;
}
...
BookMetadataCache::TocEntry Epub::getTocItem(const int tocIndex) const {
  if (!bookMetadataCache || !bookMetadataCache->isLoaded()) {
    LOG_ERR("EBP", "getTocItem called but cache not loaded");
    return {};
  }
  if (tocIndex < 0 || tocIndex >= bookMetadataCache->getTocCount()) {
    LOG_ERR("EBP", "getTocItem index:%d is out of range", tocIndex);
    return {};
  }
...
int Epub::getSpineIndexForTocIndex(const int tocIndex) const {
  ...
  if (spineIndex < 0) {
    LOG_ERR("EBP", "Section not found for TOC index %d", tocIndex);
    return 0;
  }
  return spineIndex;
}
...
int Epub::getSpineIndexForTextReference() const {
  ...
  LOG_INF("ERS", "Core Metadata: cover(%d)=%s, textReference(%d)=%s",
  ...
  for (size_t i = 0; i < getSpineItemsCount(); i++) {
    if (getSpineItem(i).href == bookMetadataCache->coreMetadata.textReferenceHref) {
      LOG_INF("ERS", "Text reference %s found at index %d", bookMetadataCache->coreMetadata.textReferenceHref.c_str(),
              i);
      return i;
    }
  }
  LOG_ERR("EBP", "Section not found for text reference");
  return 0;
}
