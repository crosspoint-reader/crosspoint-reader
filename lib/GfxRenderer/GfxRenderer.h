...
class GfxRenderer {
 ...
 public:
  ...
  // drawing them. Each {fontId, text[, style]} item that resolves to an SD
  // fallback is grouped by resolved font and prewarmed in one pass (single
  // file open, sorted seeks) instead of per-glyph through the 8-slot
  // on-demand ring. Repeat paints of the same strings cost zero SD reads.
  // No-op for strings the requested font can draw itself. Pass the style the
  // text will be DRAWN with (e.g. BOLD for header titles) — the prewarm is
  // per style.
  struct UiFallbackText {
    int fontId;
    const char* text;
    EpdFontFamily::Style style = EpdFontFamily::REGULAR;
  };
  void prewarmUiFallbackText(std::initializer_list<UiFallbackText> items) const;
