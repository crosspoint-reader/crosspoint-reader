...
class PdfCache {
 ...
 public:
  ...
  bool loadMeta(uint32_t& pageCount,
                PdfFixedVector<PdfOutlineEntry, PDF_MAX_OUTLINE_ENTRIES>& outline,
                PdfFixedVector<uint32_t, PDF_MAX_PAGES>* pageObjectIds = nullptr,
                uint32_t* fileSize = nullptr,
                uint32_t* signatureHead = nullptr,
                uint32_t* signatureTail = nullptr);
  bool saveMeta(uint32_t pageCount,
                const PdfFixedVector<PdfOutlineEntry, PDF_MAX_OUTLINE_ENTRIES>& outline,
                const PdfFixedVector<uint32_t, PDF_MAX_PAGES>* pageObjectIds = nullptr,
                uint32_t fileSize = 0,
                uint32_t signatureHead = 0,
                uint32_t signatureTail = 0);
