...
bool WebServer::_parseRequest(WiFiClient& client) {
  ...
  if (method == HTTP_POST || method == HTTP_PUT || method == HTTP_PATCH || method == HTTP_DELETE){
    ...
    if (!isForm){
      // WebDAV streaming PUT: skip reading body into RAM when there is no
      // registered handler so the onNotFound handler (WebDAV) can read it
      // directly from the TCP buffer in small chunks.
