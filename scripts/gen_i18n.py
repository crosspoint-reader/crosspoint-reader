...
def generate_keys_header(
    ...
    order_prefix = "// Order: "
    order_line = order_prefix
    for idx, name in enumerate(language_names[i] for i in sorted_indices):
        part = (", " if idx else "") + name
        if len(order_line) + len(part) > 118 and order_line != order_prefix:
