    ...
    for file in files:
        ...
        if file_lower.endswith(".html") or file_lower.endswith(".js"):
            ...
            if base_name in seen_identifiers:
                raise SystemExit(f"Error: Identifier collision detected!\n"
                               f"  Both '{seen_identifiers[base_name]}' and '{file_path}'\n"
                               f"  would generate the same base_name '{base_name}'\n"
                               f"  This would cause the second file to overwrite the first.")
            seen_identifiers[base_name] = file_path
