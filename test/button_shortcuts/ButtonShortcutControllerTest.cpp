...
TEST(ButtonShortcutControllerTest, ScreenshotChordFiresOnceAndConsumesUntilBothButtonsRelease) {
  ...
  auto result = controller.update(true, true, false, false, Controller::ChordAction::Screenshot);
  EXPECT_EQ(result.event, Controller::Event::Screenshot);
  EXPECT_TRUE(result.consumeInput);

  result = controller.update(true, true, false, false, Controller::ChordAction::Screenshot);
  EXPECT_EQ(result.event, Controller::Event::None);
  EXPECT_TRUE(result.consumeInput);

  result = controller.update(false, true, true, false, Controller::ChordAction::Screenshot);
  EXPECT_EQ(result.event, Controller::Event::None);
  EXPECT_TRUE(result.consumeInput);

  result = controller.update(false, false, false, false, Controller::ChordAction::Screenshot);
  EXPECT_EQ(result.event, Controller::Event::None);
  EXPECT_TRUE(result.consumeInput);
...
TEST(ButtonShortcutControllerTest, QuickLockChordTogglesLockState) {
  ...
  auto result = controller.update(true, true, false, false, Controller::ChordAction::QuickLock);
  EXPECT_EQ(result.event, Controller::Event::QuickLockChanged);
  EXPECT_TRUE(controller.isQuickLocked());

  (void)controller.update(false, false, false, false, Controller::ChordAction::QuickLock);
  result = controller.update(true, true, false, false, Controller::ChordAction::QuickLock);
  EXPECT_EQ(result.event, Controller::Event::QuickLockChanged);
  EXPECT_FALSE(controller.isQuickLocked());
}
...
TEST(ButtonShortcutControllerTest, PageChordActionsAreReportedWithoutChangingLockState) {
  ...
  auto result = controller.update(true, true, false, false, Controller::ChordAction::NextPage);
  EXPECT_EQ(result.event, Controller::Event::NextPage);
  EXPECT_TRUE(result.consumeInput);
  EXPECT_FALSE(controller.isQuickLocked());

  (void)controller.update(false, false, false, false, Controller::ChordAction::NextPage);
  result = controller.update(true, true, false, false, Controller::ChordAction::PreviousPage);
...
TEST(ButtonShortcutControllerTest, DisabledChordDoesNotInterceptButtons) {
  ...
  const auto result = controller.update(true, true, false, false, Controller::ChordAction::Disabled);
  EXPECT_EQ(result.event, Controller::Event::None);
  EXPECT_FALSE(result.consumeInput);
  EXPECT_FALSE(controller.isQuickLocked());
}
...
TEST(ButtonShortcutControllerTest, LockedStateSuppressesNonUnlockChordActions) {
  ...
  (void)controller.update(false, false, false, true, Controller::ChordAction::Screenshot);
  result = controller.update(true, true, false, true, Controller::ChordAction::QuickLock);
  EXPECT_EQ(result.event, Controller::Event::QuickLockChanged);
  EXPECT_TRUE(result.consumeInput);
  EXPECT_FALSE(controller.isQuickLocked());
}
